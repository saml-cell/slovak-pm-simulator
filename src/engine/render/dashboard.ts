...
function renderInstitutions(): string {
  ...
  const influenceExplain = `<div style="font-size:.65rem;color:var(--text-dim);padding:4px 6px;background:rgba(239,68,68,.06);border-left:2px solid var(--red);border-radius:3px;margin:4px 0;line-height:1.5">Tajne zatlačíte na šéfa (bývalé kauzy, priateľstvá, sľuby miesta) aby robil, čo vy chcete. Každý zásah: <strong>lojalita +2, presvedčenie −1, integrita inštitúcií −5, vzťahy s EÚ −3</strong>. Ak šéfova lojalita prekročí 8, inštitúcia sa ráta ako <em>ovládnutá</em>. Pri 4+ ovládnutých inštitúciách EÚ spustí procedúru podľa článku 7 — môžete stratiť fondy a hlasovacie práva.</div>`;
  const influenceButtons = G.politicalCapital >= 20 && influenceableHeads.length > 0
    ? `<details style="margin-top:6px"><summary style="font-size:.7rem;color:var(--gold);cursor:pointer">Ovplyvniť šéfa inštitúcie (−20 PC)</summary>${influenceExplain}${influenceableHeads.map(h => {
        const inst = era.institutions!.institutions.find(x => x.id === h.institution);
        return `<button style="display:block;width:100%;text-align:left;background:rgba(255,255,255,.03);color:#fff;border:1px solid rgba(255,255,255,.08);border-radius:4px;padding:5px 8px;font-size:.7rem;cursor:pointer;margin-top:3px" onclick="window.__influenceInstitution('${esc(h.institution)}')" title="Tajný tlak na ${esc(h.name)}. Výsledok: lojalita ${h.loyalty}→${Math.min(10, h.loyalty + 2)}, presvedčenie ${h.conviction}→${Math.max(1, h.conviction - 1)}. Cena: 20 PC + −5 integrita + −3 EÚ.">${inst ? inst.emoji + ' ' : ''}${esc(h.name)} (L:${h.loyalty})</button>`;
      }).join('')}</details>`
    : '';
